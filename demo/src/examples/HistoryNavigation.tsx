...
export const DESCRIPTION = 'Browse past positions without affecting the live game. The readonly prop adds a translucent overlay and disables interaction. Use game.history for the move list with figurine algebraic notation.';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, justifyContent: 'center', marginTop: 8 }}>
        <button onClick={() => setViewPly(0)} disabled={game.history.length === 0}>|&lt;</button>
        <button onClick={() => {
          const cur = viewPly ?? game.history.length;
          if (cur > 0) setViewPly(cur - 1);
        }} disabled={game.history.length === 0}>&lt;</button>
        <button onClick={() => {
          if (viewPly !== null) {
            setViewPly(viewPly + 1 >= game.history.length ? null : viewPly + 1);
          }
        }} disabled={viewPly === null}>&gt;</button>
        <button onClick={() => setViewPly(null)} disabled={viewPly === null}>&gt;|</button>
      </div>
      <div style={{ marginTop: 8, fontSize: 14, lineHeight: 1.8, color: '#ccc' }}>
        {game.history.length === 0 && <span style={{ color: '#666' }}>Play some moves to see history</span>}
        {game.history.map((e, i) => (
          <span key={i} onClick={() => setViewPly(i + 1)}
            style={{
              cursor: 'pointer',
